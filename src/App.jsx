...
function blockScores(items, bloques, answers){
  ...
  // Alerta de bienestar anímico (bloque I, solo Cuestionario 2): media baja
  // dispara el mismo mecanismo de alerta que los ítems ALERTA_. Umbral 2.0
  // sobre 4 (equivalente a responder "a veces" o menos en promedio) — es un
  // criterio de cribado interno, ajústalo si tu criterio profesional pide otro.
  if (out.I != null && out.I <= 2) out["ALERTA_ANIMO"] = true;
  return out;
}

// Único punto de verdad para "¿esta respuesta dispara alerta?": cubre tanto
// los ítems ALERTA_ (conducta agresora, valor bruto 1-4) como ALERTA_ANIMO
// (media baja en bienestar anímico, booleano). Evita tener el mismo criterio
// repetido y potencialmente desincronizado en varios sitios del panel.
...
function InformeIndividual({ r, groupAvg, cfg, onVolver }){
  ...
  const esAlertaConducta = Object.entries(r.scores||{}).some(([k,v]) => k.startsWith("ALERTA_") && k !== "ALERTA_ANIMO" && v>=3);
  const esAlertaAnimo = r.scores?.ALERTA_ANIMO === true;
  return (
    <div>
      ...
      <div style={{border:"1px solid #12414f", padding:24, background:"#fff"}}>
        ...
        {esAlertaAnimo && (
          <div style={{background:"#f7e9e5", border:"1px solid #c2694a", padding:10, borderRadius:3, fontSize:13, marginBottom:16}}>
            ⚠ Puntuación baja en bienestar anímico (tristeza, desánimo, energía, concentración). Se recomienda valorar una conversación individual antes de compartir el informe.
          </div>
        )}
